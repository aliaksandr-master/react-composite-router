...
export const routeCreator = (func, parentRoute = null) => ({
  create: (name, definition) => {
    const route = func(name, definition, parentRoute);
    ...
    return routeCreator(func, route);
  }
});


const createTreeOfRoutes = (name, { url = '', params = {}, slots = {} }, parent = null) => {
  // pre checking
...
export const route = routeCreator(routesByName, routesNamesInSearchingOrder, createTreeOfRoutes, null);
...
export const calcLocation = (routesMap, state, ...someParams) => {
  if (!routesMap.hasOwnProperty(state)) {
    throw new Error(`route "${state}" not found`);
  }

  const route = routesMap[state];
